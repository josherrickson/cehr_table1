...
{pstd}
Diff-in-diff tables can be generated by specifying two separate variables inside {cmd:by()}. The first variable is the upper
split and the second variable is the lower split.
...
{pstd}
For diff-in-diff tables, the lower split (2nd entry in {cmd:by()} must have exactly two levels for standardized difference
to be printed.
...
{pstd}A diff-in-diff table{p_end}
{phang2}{stata webuse productivity:. webuse productivity}{p_end}
{phang2}{stata gen timeperiod = year < 1980:. gen timeperiod = year < 1980}{p_end}
{phang2}{stata tab timeperiod:. tab timeperiod}{p_end}
{phang2}{stata cehr_table1 _samplesize water hwy using ~/Desktop/tmp.xlsx, by(timeperiod region):. cehr_table1 _samplesize water hwy using ~/Desktop/tmp.xlsx, by(timeperiod region)}{p_end}
